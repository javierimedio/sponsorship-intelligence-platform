...
export async function POST(_request: NextRequest, { params }: RouteParams) {
  ...
  if (proposal.finalized_at) {
    return NextResponse.json({ error: 'Esta propuesta ya estaba finalizada.' }, { status: 409 });
  }
  if (!proposal.approved_at) {
    return NextResponse.json({ error: 'Aprueba la propuesta antes de finalizarla.' }, { status: 400 });
  }
