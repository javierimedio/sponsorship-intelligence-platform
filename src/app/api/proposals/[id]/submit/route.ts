...
export async function POST(_request: NextRequest, { params }: RouteParams) {
  ...
  const { data: proposal, error: fetchError } = await supabase
    .from('proposals')
    .select('id, recommendation, submitted_at')
    .eq('id', params.id)
    .maybeSingle();
  ...
  if (proposal.submitted_at) {
    return NextResponse.json({ error: 'Esta propuesta ya estaba enviada.' }, { status: 409 });
  }
  if (!proposal.recommendation) {
    return NextResponse.json(
      { error: 'Evalúa la propuesta (scoring/riesgo/financials) antes de enviarla.' },
      { status: 400 },
    );
  }
